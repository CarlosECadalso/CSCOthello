 ...
 *
 */
public class Othello implements Visitable {
	...
	public boolean move(int row, int col) {
		OthelloMoveVisitor mv = new OthelloMoveVisitor(row, col, this.whosTurn);
		this.board.accept(mv);
		if (mv.getMoved()) {
			this.whosTurn = OthelloBoard.otherPlayer(this.whosTurn);
			HasMoveVisitor hmv = new HasMoveVisitor();
			this.board.accept(hmv);
			char allowedMove = hmv.getToken();
